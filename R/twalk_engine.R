...
# Constantes globais internas ao pacote.
# O R lida com isso corretamente durante a construção do pacote.
log_2pi <- log(2*pi)
log_3 <- log(3)
...
executar_movimento <- function(dimensao, funcao_objetivo, funcao_suporte, x, valor_U, x_prime, valor_Up,
  ...
  # (O corpo da sua função `executar_movimento` entra aqui, sem nenhuma alteração)
  # Apenas certifique-se de que as chamadas a funções como runif, rnorm, etc.,
  # tenham o prefixo do pacote, como `stats::runif()`.
